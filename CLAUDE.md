...
| Judge-парсинг | **отдельный LLM-вызов** (gpt-4o-mini structured output) | Надёжнее regex, стоимость в бюджете |
...
```
Inngest cron (weekly/daily) 
    └─► create_run(brand)
          └─► fan-out: query × provider × replication
                └─► [check daily cost cap] ───► abort if over
                └─► LLM call (retry, timeout)
                      └─► parse_mention()        -- отдельный LLM-judge
                            └─► save_result()
          └─► aggregate_run()                   -- majority vote
                └─► delta_detect()              -- сравнение с прошлым run
                      └─► notify()              -- Resend email
```
